...
object ListSetLemmas {
  ...
  def check_is_member[T] (e : T, s : List[T], t : List[T]) : Boolean = {
    ...
    ListSetOps.is_member(e, s) == ListSetOps.is_member(e, t)
  }.holds

  @induct
  def insert_distinct[T] (e : T, set : List[T]) : Boolean = {
    require(distinct(set))
    distinct(ListSetOps.insert(e, set))
  }.holds

  def check_insert[T] (e : T, s : List[T], t : List[T]) : Boolean = {
    require(distinct(s) && distinct(t) && s.content == t.content)
    val u = ListSetOps.insert(e, s)
    val v = ListSetOps.insert(e, t)
  ...
  def remove_distinct[T] (e : T, set : List[T]) : Boolean = {
    require(distinct(set))
    distinct(ListSetOps.remove(e, set))
  }.holds

  def check_remove[T] (e : T, s : List[T], t : List[T]) : Boolean = {
    require(distinct(s) && distinct(t) && s.content == t.content)
    val u = ListSetOps.remove(e, s)
    val v = ListSetOps.remove(e, t)
    distinct(u) && distinct(v) && u.content == v.content because
  ...
  def insertAll_distinct[T] (es : List[T], set : List[T]) : Boolean = {
    require(distinct(set))
    if (es.isEmpty) {
      distinct(ListSetOps.insertAll(es, set))
    } else {
      distinct(ListSetOps.insertAll(es, set)) because {
        distinct(ListSetOps.insertAll(es.tail, set)) &&
          insert_distinct(es.head, ListSetOps.insertAll(es.tail, set))
      }
    }
  }.holds
  ...
  def check_insertAll[T] (es : List[T], s : List[T], t : List[T]) : Boolean = {
    require(distinct(s) && distinct(t) && s.content == t.content)
    val u = ListSetOps.insertAll(es, s)
    val v = ListSetOps.insertAll(es, t)
    distinct(u) && distinct(v) && u.content == v.content because
  ...
  def check_union[T] (s0 : List[T], s1 : List[T], t0 : List[T], t1 : List[T]) : Boolean = {
    ...
    val u0 = ListSetOps.union(s0, t0)
    val u1 = ListSetOps.union(s1, t1)
