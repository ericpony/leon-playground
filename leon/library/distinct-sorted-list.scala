...
object DistinctSortedListLemmas {
  ...
  @induct
  def merge_distinct (l1 : List[BigInt], l2 : List[BigInt]) : Boolean = {
    require (distinct_sorted (l1) && distinct_sorted (l2))
    if (l1 == Nil[BigInt]() || l2 == Nil[BigInt]()) {
      ...
      if (h1 < h2) {
        ...
        if (h2 < h1) {
          distinct (merge (l1, l2)) because {
            distinct_cons_not_contains (merge (l1, l2.tail), h2) because {
              merge_not_contains (l1, l2.tail, h2) because {
                merge_distinct (l1, l2.tail) &&
                !l1.contains (h2) because {
                  sort_head_min (l1) && min_not_contains (l1, h2) }
              }
            }
          }
        } else {
  ...
  def distinct_content_permutation (l1 : List[BigInt], l2 : List[BigInt]) : Boolean = {
    ...
    if (l1 == Nil[BigInt]()) {
      ...
      permutation (l1, l2) because {
        check { distinct_delete (l2, h1) } &&
        check { distinct_delete_content (l1, l2, h1) } &&
        check { distinct_content_permutation (l1.tail, delete (l2, h1)) }
      }
    }
  } holds
